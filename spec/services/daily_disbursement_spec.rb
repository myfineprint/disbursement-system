...
RSpec.describe DailyDisbursement do
  ...
  let(:daily_disbursement) do
    described_class.new(merchant: merchant, orders: orders)
  end
  ...
  describe '#call' do
    context 'with valid data' do
      ...
      it 'creates disbursement with correct calculated amounts' do
        ...
        expect(result.total_net_amount).to eq(
          result.total_gross_amount - result.total_commission
        )
      end
      ...
      it 'generates a valid reference' do
        ...
        expect(result.reference).to match(
          /^D-#{merchant.reference}-\d{8}-\d{4}$/
        )
    ...
    context 'with edge case order amounts' do
      it 'handles very small amounts' do
        ...
        daily_disbursement =
          described_class.new(merchant: merchant, orders: [small_order])
      ...
      it 'handles very large amounts' do
        ...
        daily_disbursement =
          described_class.new(merchant: merchant, orders: [large_order])
      ...
      it 'handles zero amount orders' do
        ...
        daily_disbursement =
          described_class.new(merchant: merchant, orders: [zero_order])
  ...
  describe 'idempotency' do
    it 'raises error when called multiple times with same data due to unique constraints' do
      daily_disbursement.call

      # Second call should fail due to unique constraint violation
      expect { daily_disbursement.call }.to raise_error(
        ActiveRecord::RecordNotUnique
      )

      # Should only have one disbursement and no duplicate associations
      expect(Disbursement.count).to eq(1)
      expect(DisbursementOrder.count).to eq(orders.length)
    end
  end
end
